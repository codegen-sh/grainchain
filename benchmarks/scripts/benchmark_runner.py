...
class BenchmarkRunner:
    ...
    def _load_config(self, config_path: str) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            "trivial_changes": [
                {"type": "comment", "file": "README.md", "content": "# Benchmark test comment"},
                {"type": "whitespace", "file": "package.json", "content": "\n"},
                {"type": "log", "file": "app/index.js", "content": "console.log('benchmark test');"}
            ]
        }
    ...
    def setup_container(self) -> bool:
        ...
        try:
            ...
            self.container = self.docker_client.containers.run(
                ...
                volumes={
                    str(Path.cwd()): {"bind": "/host", "mode": "rw"}
                }
            )
    ...
    def clone_and_install_outline(self) -> bool:
        ...
        try:
            ...
            result = self.container.exec_run(clone_cmd, workdir=self.config["workspace_path"])
            if result.exit_code != 0:
                self.logger.error(f"Failed to clone repository: {result.output.decode()}")
                return False
            
            # Checkout specific branch if needed
            if self.config["outline_branch"] != "main":
                checkout_cmd = f"git checkout {self.config['outline_branch']}"
                result = self.container.exec_run(checkout_cmd, workdir=f"{self.config['workspace_path']}/outline")
                if result.exit_code != 0:
                    self.logger.error(f"Failed to checkout branch: {result.output.decode()}")
                    return False
            ...
            if result.exit_code != 0:
                self.logger.error(f"Failed to install dependencies: {result.output.decode()}")
                return False
    ...
    def take_snapshot(self, snapshot_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # File system metrics
            result = self.container.exec_run("du -sh outline", workdir=self.config["workspace_path"])
            if result.exit_code == 0:
            ...
            # Node modules size
            result = self.container.exec_run("du -sh outline/node_modules", workdir=self.config["workspace_path"])
            if result.exit_code == 0:
                size_output = result.output.decode().strip()
                snapshot["metrics"]["filesystem"]["node_modules_size"] = size_output.split()[0]
            
            # Package count
            result = self.container.exec_run("find outline/node_modules -name package.json | wc -l", workdir=self.config["workspace_path"])
            if result.exit_code == 0:
            ...
            result = self.container.exec_run("yarn build", workdir=f"{self.config['workspace_path']}/outline")
            ...
            if result.exit_code != 0:
                snapshot["metrics"]["performance"]["build_error"] = result.output.decode()
            
            # Test run time (if tests exist)
            start_time = time.time()
            result = self.container.exec_run("yarn test --passWithNoTests", workdir=f"{self.config['workspace_path']}/outline")
    ...
    def apply_trivial_change(self, change: Dict[str, str]) -> bool:
        ...
        try:
            ...
            result = self.container.exec_run(cmd, workdir=f"{self.config['workspace_path']}/outline")
    ...
    def revert_changes(self) -> bool:
        ...
        try:
            ...
            result = self.container.exec_run("git checkout .", workdir=f"{self.config['workspace_path']}/outline")
            return result.exit_code == 0
        except Exception as e:
    ...
    def run_benchmark(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(self.config["benchmark_iterations"]):
                self.logger.info(f"Running benchmark iteration {i + 1}/{self.config['benchmark_iterations']}")
    ...
    def _compare_snapshots(self, baseline: Dict[str, Any], current: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "metrics" in baseline and "metrics" in current:
            # Compare build times
            if "performance" in baseline["metrics"] and "performance" in current["metrics"]:
                baseline_build = baseline["metrics"]["performance"].get("build_time_seconds", 0)
                current_build = current["metrics"]["performance"].get("build_time_seconds", 0)
                
                comparison["differences"]["build_time_change"] = current_build - baseline_build
                comparison["differences"]["build_time_percent_change"] = (
                    ((current_build - baseline_build) / baseline_build * 100) if baseline_build > 0 else 0
                )
            
            # Compare memory usage
            if "container" in baseline["metrics"] and "container" in current["metrics"]:
                baseline_memory = baseline["metrics"]["container"].get("memory_usage", 0)
                current_memory = current["metrics"]["container"].get("memory_usage", 0)
                
                comparison["differences"]["memory_change"] = current_memory - baseline_memory
...
"""
        ...
        for snapshot in results.get("snapshots", []):
            if "metrics" in snapshot:
                build_time = snapshot["metrics"].get("performance", {}).get("build_time_seconds", "N/A")
                memory_mb = round(snapshot["metrics"].get("container", {}).get("memory_usage", 0) / 1024 / 1024, 2)
                package_count = snapshot["metrics"].get("filesystem", {}).get("package_count", "N/A")
                build_success = snapshot["metrics"].get("performance", {}).get("build_success", False)
