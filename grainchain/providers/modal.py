...
try:
    import modal
    from modal import App, Sandbox as ModalSandbox
    ...
    async def _create_session(self, config: SandboxConfig) -> "ModalSandboxSession":
        ...
        try:
            ...
            image = modal.Image.from_registry(
                config.image or self.image_name
            )
            ...
            raise ProviderError(f"Modal sandbox creation failed: {e}", self.name, e)
...
class ModalSandboxSession(BaseSandboxSession):
    ...
    async def download_file(self, path: str) -> bytes:
        ...
        try:
            ...
            if not result.success:
                raise ProviderError(f"Failed to read file: {result.stderr}", self._provider.name)
    ...
    async def create_snapshot(self) -> str:
        ...
        try:
            ...
            await self.execute(f"echo 'Snapshot {snapshot_id} created at {time.time()}' > /tmp/grainchain_snapshot_{snapshot_id}")
            return snapshot_id
    ...
    async def restore_snapshot(self, snapshot_id: str) -> None:
        ...
        raise NotImplementedError("Modal provider does not support snapshot restoration")
