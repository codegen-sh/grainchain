...
class LocalProvider(BaseSandboxProvider):
    ...
    async def _create_session(self, config: SandboxConfig) -> "LocalSandboxSession":
        ...
        try:
            ...
            raise ProviderError(f"Local sandbox creation failed: {e}", self.name, e)
    ...
    async def execute(
        ...
        try:
            ...
            try:
                ...
                process.kill()
                await process.wait()
                raise ProviderError(f"Command timed out after {timeout or self.config.timeout} seconds", self._provider.name)
    ...
    async def restore_snapshot(self, snapshot_id: str) -> None:
        ...
        try:
            if snapshot_id not in self._snapshots:
                raise ProviderError(f"Snapshot not found: {snapshot_id}", self._provider.name)
            
            snapshot_dir = self._snapshots[snapshot_id]
            
            if not os.path.exists(snapshot_dir):
                raise ProviderError(f"Snapshot directory not found: {snapshot_dir}", self._provider.name)
            ...
            raise ProviderError(f"Snapshot restoration failed: {e}", self._provider.name, e)
