    ...
    async def _create_session(self, config: SandboxConfig) -> "DaytonaSandboxSession":
        ...
        try:
            ...
            if "authentication" in str(e).lower() or "unauthorized" in str(e).lower():
                raise AuthenticationError(f"Daytona authentication failed: {e}", self.name)
            else:
                raise ProviderError(f"Daytona sandbox creation failed: {e}", self.name, e)
...
class DaytonaSandboxSession(BaseSandboxSession):
    ...
    async def create_snapshot(self) -> str:
        ...
        try:
            ...
            await self.execute(f"echo 'Snapshot created at {time.time()}' > /tmp/grainchain_snapshot_{snapshot_id}")
    ...
    async def restore_snapshot(self, snapshot_id: str) -> None:
        ...
        raise NotImplementedError("Daytona provider does not support snapshot restoration")
