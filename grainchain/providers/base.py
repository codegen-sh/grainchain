...
from grainchain.core.exceptions import ConfigurationError, ProviderError
...
class BaseSandboxProvider(SandboxProvider):
    ...
    async def create_sandbox(self, config: SandboxConfig) -> SandboxSession:
        ...
        try:
            ...
            logger.info(
                f"Created sandbox {session.sandbox_id} with provider {self.name}"
            )
            return session
        except Exception as e:
            logger.error(f"Failed to create sandbox with provider {self.name}: {e}")
            raise ProviderError(f"Failed to create sandbox: {e}", self.name, e) from e
    ...
    def require_config_value(self, key: str) -> Any:
        ...
        if value is None:
            raise ConfigurationError(
                f"Required configuration '{key}' not found for provider {self.name}"
            )
        return value
...
class BaseSandboxSession(SandboxSession):
    ...
    def __init__(
        self, sandbox_id: str, provider: BaseSandboxProvider, config: SandboxConfig
    ):
        """
    ...
    def _set_status(self, status: SandboxStatus) -> None:
        ...
        if old_status != status:
            logger.debug(
                f"Sandbox {self.sandbox_id} status changed: {old_status.value} -> {status.value}"
            )
    ...
    def _ensure_not_closed(self) -> None:
        ...
        if self._closed:
            raise ProviderError(
                f"Sandbox {self.sandbox_id} is closed", self._provider.name
            )
    ...
    async def create_snapshot(self) -> str:
        ...
        raise NotImplementedError(
            f"Snapshots not supported by provider {self._provider.name}"
        )
